...
export class G964 {
    ...
    public static travel = (r: string, zipcode: string) => {
        ...
        if (!foundLocations || !zipcode) return `${zipcode}:/`;

        const zipCodes = foundLocations.map(locationStr => locationStr.split(' ')[0]).join(',');
        const locationsString = foundLocations.map(locationStr => {
            const locStrArray = locationStr.split(' ');
            return locStrArray.slice(1,locStrArray.length-2).join(' ');
        });

        return `${zipcode}:${locationsString}/${zipCodes}`;
    }
}

console.log(G964.travel(ad, "AA 45522"));
// console.log(G964.travel(ad, "EX 34342"));
// console.log(G964.travel(ad, "EX 34345"));
// console.log(G964.travel(ad, "AA 45521"));
// console.log(G964.travel(ad, "AE 56215"));
// console.log(G964.travel(ad, ""));
